        ...
        while changed_number:
            ...
            for i in empty:
                ...
                if len(position_actions)==2 and 2 not in self.board.adjacent_horizontal_numbers(*i) and np.any(self.rows[i[1]]==half-1):
                    ...
                    elif self.rows[i[1,1]]==half-1: #quer dizer que é o 1 que apenas falta acrescentar 1
                        onde_dois = np.argwhere(test_row==2)
                        dois = np.count_nonzero(test_row ==2)
                ...
                if len(position_actions)==2 and 2 not in self.board.adjacent_vertical_numbers(*i) and np.any(self.cols[i[0]]==half-1):
                    ...
                    elif self.cols[i[0,1]]==half-1: #quer dizer que é o 1 que apenas falta acrescentar 1
                        onde_dois = np.argwhere(test_col==2)
                        dois = np.count_nonzero(test_col==2)
